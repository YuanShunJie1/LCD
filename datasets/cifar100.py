...
class Cifar100Setup(DatasetSetup):
    ...
    def set_datasets_for_ssl(self, file_path, n_labeled, party_num):
        ...
        base_dataset = torchvision.datasets.CIFAR100(file_path, train=True, download=True)
    ...
    def get_transformed_dataset(self, file_path, party_num=None, train=True):
        ...
        transforms_ = self.get_transforms()
        _cifar100_dataset = datasets.CIFAR100(file_path, train, download=True, transform=transforms_)

        labels = _cifar100_dataset.targets
        # labels_list = labels.tolist()      
        labels_list = labels if isinstance(labels, list) else labels.tolist()
        return _cifar100_dataset, labels_list
