...
# # dataset paras
# parser.add_argument('-d', '--dataset', default='Criteo', type=str, help='name of dataset' )
# parser.add_argument('--path-dataset', help='path_dataset', type=str, default='D:/Datasets/yahoo_answers_csv/')
...
class VflFramework(nn.Module):
    def __init__(self):
        ...
        if args.use_mal_optim_top:
            self.optimizer_top_model = my_optimizers.MaliciousSGD(self.top_model.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
        else:
...
def set_loaders():
    ...
    train_dataset, _ = dataset_setup.get_transformed_dataset(args.path_dataset, None, True)
    test_dataset, _ = dataset_setup.get_transformed_dataset(args.path_dataset, None, False)
    ...
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=args.batch_size,num_workers=args.workers)
...
def main():
    ...
    lr_scheduler_top_model = torch.optim.lr_scheduler.MultiStepLR(model.optimizer_top_model, milestones=[stone1, stone2], gamma=args.step_gamma)
    lr_scheduler_m_a = torch.optim.lr_scheduler.MultiStepLR(model.optimizer_malicious_bottom_model_a, milestones=[stone1, stone2], gamma=args.step_gamma)
    lr_scheduler_b_b = torch.optim.lr_scheduler.MultiStepLR(model.optimizer_benign_bottom_model_b, milestones=[stone1, stone2], gamma=args.step_gamma)
    ...
    # save model
    torch.save(model, os.path.join(dir_save_model, f"{args.dataset}_saved_framework{setting_str}.pth"), pickle_module=dill)
